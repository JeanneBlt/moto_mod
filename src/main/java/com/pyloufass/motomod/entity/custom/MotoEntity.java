...
public class MotoEntity extends AbstractMotoEntity {
    ...
    public static final TrackedData<Boolean> DASHING = DataTracker.registerData(MotoEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    public static final TrackedData<Long> LAST_POSE_TICK = DataTracker.registerData(MotoEntity.class, TrackedDataHandlerRegistry.LONG);
    ...
    private static final TrackedData<Integer> DATA_ID_TYPE_VARIANT =
            DataTracker.registerData(MotoEntity.class, TrackedDataHandlerRegistry.INTEGER);
    private static final EntityDimensions SITTING_DIMENSIONS = EntityType.HORSE
            .getDimensions()
    ...
    public MotoEntity(EntityType<? extends MotoEntity> entityType, World world) {
    ...
    public static DefaultAttributeContainer.Builder createMotoAttributes() {
    ...
    public MotoVariant getVariant() {
        return MotoVariant.byId(this.getTypeVariant() & 255);
    }
    ...
    private void setVariant(MotoVariant variant) {
    ...
    public static boolean canSpawn(EntityType<MotoEntity> type, WorldAccess world, SpawnReason reason, BlockPos pos, Random random) {
    ...
    protected void mobTick(ServerWorld world) {
        ...
        ((Brain<MotoEntity>)brain).tick(world, this);
        profiler.pop();
    ...
    public boolean canBreedWith(AnimalEntity other) {
        return other != this && other instanceof MotoEntity camelEntity && this.canBreed() && camelEntity.canBreed();
    }
    ...
    protected BodyControl createBodyControl() {
        return new MotoEntity.CamelBodyControl(this);
    }

    class CamelBodyControl extends BodyControl {
        public CamelBodyControl(final MotoEntity camel) {
            super(camel);
        }

        @Override
        public void tick() {
            if (!MotoEntity.this.isStationary()) {
                super.tick();
            }
        }
    }
    ...
    class CamelLookControl extends LookControl {
        CamelLookControl() {
            super(MotoEntity.this);
        }

        @Override
        public void tick() {
            if (!MotoEntity.this.hasControllingPassenger()) {
                super.tick();
            }
        }
    }
    ...
    class CamelMoveControl extends MoveControl {
        ...
        public void tick() {
            if (this.state == MoveControl.State.MOVE_TO
                    && !MotoEntity.this.isLeashed()
                    && MotoEntity.this.isSitting()
                    && !MotoEntity.this.isChangingPose()
                    && MotoEntity.this.canChangePose()) {
                MotoEntity.this.startStanding();
            }
